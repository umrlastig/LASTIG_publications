...
```{r}
res <- thematicMap(M_wos, field = "DE", n = 250, minfreq = 5, size = 0.5, repel = TRUE, synonyms = synonyms_file)
plot(res$map)
```
...
```{r}
res <- thematicMap(M_scopus, field = "DE", n = 250, minfreq = 5, size = 0.5, repel = TRUE, synonyms = synonyms_file)
plot(res$map)
```
...
```{r}
resW <- wordlist(M=M_wos, Field="ID", n=500, measure="identity", ngrams=1, remove.terms = NULL, synonyms = synonyms_file)
W <- resW$W
write.csv(resW[["W"]], "/home/JPerret/data_wos.csv", row.names=FALSE)
Words <- resW$Words
wordcloud2::wordcloud2(W, size = 1.5, minRotation = -pi/2, maxRotation = -pi/2)
```

Wordcloud with WoS and author's keywords.
```{r}
resW <- wordlist(M=M_wos, Field="DE", n=500, measure="identity", ngrams=1, remove.terms = NULL, synonyms = synonyms_file)
W <- resW$W
write.csv(resW[["W"]], "/home/JPerret/data_wos_de.csv", row.names=FALSE)
Words <- resW$Words
wordcloud2::wordcloud2(W, size = 1.5, minRotation = -pi/2, maxRotation = -pi/2)
```
...
```{r}
resW <- wordlist(M=M_scopus, Field="ID", n=500, measure="identity", ngrams=1, remove.terms = NULL, synonyms = synonyms_file)
W <- resW$W
write.csv(resW[["W"]], "/home/JPerret/data_scopus.csv", row.names=FALSE)
Words <- resW$Words
wordcloud2::wordcloud2(W, size = 1.5, color = 'random-dark',minRotation = 0, maxRotation = 0, rotateRatio = 1)
```

Wordcloud with Scopus and author's keywords.
```{r}
resW <- wordlist(M=M_scopus, Field="DE", n=500, measure="identity", ngrams=1, remove.terms = NULL, synonyms = synonyms_file)
W <- resW$W
write.csv(resW[["W"]], "/home/JPerret/data_scopus_de.csv", row.names=FALSE)
Words <- resW$Words
wordcloud2::wordcloud2(W, size = 1.5, color = 'random-dark',minRotation = 0, maxRotation = 0, rotateRatio = 1)
```
